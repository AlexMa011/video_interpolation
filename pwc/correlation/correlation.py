...
kernel_SpatialConvolve_updateGradFirst = '''
	...
	extern "C" __global__ void kernel_SpatialConvolve_updateGradFirst(
	  ...
	  float* gradFirst,
	  float* gradSecond
	) { for (int intIndex = (blockIdx.x * blockDim.x) + threadIdx.x; intIndex < n; intIndex += blockDim.x * 
	gridDim.x) {
	  ...
	  // We add round_off before_s1 the int division and subtract round_off after it, to ensure the formula matches 
	  ceil behavior:
	      ...
	      for (int o = -4; o <= 4; o++) {
	        ...
	        for (int y = ymin; y <= ymax; y++) {
	          for (int x = xmin; x <= xmax; x++) {
	            int idxgradOutput = (idxopoffset * SIZE_2(gradOutput) + y) * SIZE_3(gradOutput) + x; // gradOutput[x,
	            y,o,p]
	...
	gridDim.x) {
	  ...
	  for (int p = -4; p <= 4; p++) {
	    for (int o = -4; o <= 4; o++) {
	      ...
	      // We add round_off before_s1 the int division and subtract round_off after it, to ensure the formula 
	      matches ceil behavior:
	  ...
	  gradSecond[bot1index + intSample*SIZE_1(gradSecond)*SIZE_2(gradSecond)*SIZE_3(gradSecond)] = sum / (
	  float)sumelems;
	} }
'''
        ...
        if first.is_cuda == True:
            if gradFirst is not None:
                for intSample in range(first.size(0)):
                    ...
                    cupy_launch('kernel_SpatialConvolve_updateGradFirst',
                                cupy_kernel('kernel_SpatialConvolve_updateGradFirst', {
                                    'rbot0': self.rbot0,
                                    'rbot1': self.rbot1,
                        ...
                        block=tuple([512, 1, 1]),
                        args=[n, intSample, self.rbot0.data_ptr(), self.rbot1.data_ptr(), gradOutput.data_ptr(),
            ...
            if gradSecond is not None:
                for intSample in range(first.size(0)):
                    n = first.size(1) * first.size(2) * first.size(3)
                    cupy_launch('kernel_SpatialConvolve_updateGradSecond',
                                cupy_kernel('kernel_SpatialConvolve_updateGradSecond', {
                                    'rbot0': self.rbot0,
                                    'rbot1': self.rbot1,
                                    'gradOutput': gradOutput,
                                    'gradFirst': None,
                        ...
                        grid=tuple([int((n + 512 - 1) / 512), 1, 1]),
                        block=tuple([512, 1, 1]),
                        args=[n, intSample, self.rbot0.data_ptr(), self.rbot1.data_ptr(), gradOutput.data_ptr(), None,
                              gradSecond.data_ptr()],
                        stream=Stream
                    )
            # end
