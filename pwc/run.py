...
# for strOption, strArgument in getopt.getopt(sys.argv[1:], '',
...
# 	elif strOption == '--out':
# 		arguments_strOut = strArgument # path to where the output should be
# stored
    ...
    def __init__(self):
        ...
        class Extractor(torch.nn.Module):
            ...
            def forward(self, tensorInput):
                ...
                return [tensorOne, tensorTwo, tensorThr, tensorFou, tensorFiv,
                        tensorSix]
        ...
        class Backward(torch.nn.Module):
            ...
            def forward(self, tensorInput, tensorFlow):
                if hasattr(self,
                           'tensorPartial') == False or \
                    self.tensorPartial.size(
                        0) != tensorFlow.size(0) or self.tensorPartial.size(
                    2) != tensorFlow.size(2) or self.tensorPartial.size(
                    3) != tensorFlow.size(3):
                    self.tensorPartial = tensorFlow.new_ones(
                        tensorFlow.size(0), 1, tensorFlow.size(2),
                        tensorFlow.size(3))
                # end

                if hasattr(self,
                           'tensorGrid') == False or self.tensorGrid.size(
                    0) != tensorFlow.size(0) or self.tensorGrid.size(
                    2) != tensorFlow.size(2) or self.tensorGrid.size(
                    3) != tensorFlow.size(3):
                    tensorHorizontal = torch.linspace(-1.0, 1.0,
                                                      tensorFlow.size(3)).view(
                        1, 1, 1, tensorFlow.size(3)).expand(tensorFlow.size(0),
                                                            -1,
                                                            tensorFlow.size(2),
                                                            -1)
                    tensorVertical = torch.linspace(-1.0, 1.0,
                                                    tensorFlow.size(2)).view(1,
                                                                             1,
                                                                             tensorFlow.size(
                                                                                 2),
                                                                             1).expand(
        ...
        class Decoder(torch.nn.Module):
            def __init__(self, intLevel):
                ...
                self.moduleCorreleaky = torch.nn.LeakyReLU(inplace=False,
                                                           negative_slope=0.1)
            ...
            def forward(self, tensorFirst, tensorSecond, objectPrevious):
                ...
                if objectPrevious is None:
                    ...
                    tensorVolume = self.moduleCorreleaky(
                        self.moduleCorrelation(tensorFirst,
                                               self.moduleBackward(
                                                   tensorSecond,
                                                   tensorFlow *
                                                   self.dblBackward)))
        ...
        self.load_state_dict(torch.load(
            '/home/alex/projects/work/pwc/models/default.pytorch'))
...
def estimate(tensorFirst, tensorSecond, moduleNetwork, devices=0):
    ...
    # make sure to not compute gradients for computational
    # performance

    torch.cuda.device(devices)
    # change this if you have a multiple graphics cards and
    # you want to utilize them

    torch.backends.cudnn.enabled = True  # make sure to use cudnn for
    ...
    # assert (
    #         intWidth == 1024)  # remember that there is no guarantee for
    # correctness, comment this line out if you acknowledge this and want to
    #  continue
    # assert (
    #         intHeight == 436)  # remember that there is no guarantee for
    # correctness, comment this line out if you acknowledge this and want to
    #  continue
...
# 	tensorFirst = torch.FloatTensor(numpy.array(PIL.Image.open(
# arguments_strFirst))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32)
# * (1.0 / 255.0))
# 	tensorSecond = torch.FloatTensor(numpy.array(PIL.Image.open(
# arguments_strSecond))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32)
#  * (1.0 / 255.0))
...
# 	numpy.array([ tensorOutput.size(2), tensorOutput.size(1) ],
